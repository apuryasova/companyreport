...
fake = Faker()
"""
Текущий тест сталкивается с проблемами при работе с меню навигации. Локаторы корректны, но поведение элементов в меню нестабильно, что приводит к ошибкам. Рекомендуется:
1. Уточнить идентификаторы элементов меню.
2. Проверить логику отображения элементов (возможны перекрытия или задержки).
Дальнейшее тестирование невозможно без доработок в системе. """
